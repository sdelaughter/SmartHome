...
#The persistent state of the bulb.  0=off 1=on
STATE = 0
...
#Change the state of the bulb
	global STATE
	STATE = s
	logging.info('State was set to: ' + str(s))
...
def main():
	#Set up logging
	logging.basicConfig(filename='logs/bulb.log', filemode='w', level=logging.DEBUG, format='%(asctime)s.%(msecs)3d | %(levelname)8s | %(funcName)s: %(message)s', datefmt="%Y-%m-%d %H:%M:%S")
