...
#Read the gateway addresses from the gateway_list.txt file and return one at random
	read_address = False
	while not read_address:
		try:
			with open('gateway_list.txt', 'r') as f:
				addresses = f.readlines()
			logging.debug('Read gateway addresses from file: ' + str(addresses))
			read_address = True
		except:
	...
	logging.debug('Returning randomly selected address: ' + str(address))
	return(address)


def get_gateway_addresses():
#Read the gateway addresses from the gateway_list.txt file and return one at random
	read_address = False
	while not read_address:
		try:
...
#The main class for all IOT devices, all other device classes inherit from this
	...
	#Find all devices (other than self) in self.devices with a given name
	
		device_list = []
		for i in self.devices:
			d = self.devices[i]
			if(d['name'] == name):
				if((d['ip'] == self.ip) and (d['port'] == self.port) and (d['name'] == self.name)):
					pass
				else:
	...
	#Find all devices (other than self) in self.devices with a given name and return one at random
	#Useful for load-balancing replicas of gateways and databases
	
		device_list = []
		for i in self.devices:
			if(self.devices[i]['name'] == name):
				if((self.devices[i]['ip'] == self.ip) and (self.devices[i]['port'] == self.port)):
					pass
				else:
					device_list.append(self.devices[i])
	...
	#Check the local cache first, then retrieve from a random database if the query misses.
		...
		dbs = self.device_by_name('backend')
		values = []
		for db in dbs:
			self.clock += 1
			r = build_receiver(db)
